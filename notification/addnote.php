 ...
 * after looking at there performance
 * @package usp_ews
 */
...
$url = new moodle_url('/blocks/usp_ews/notification/addnote.php', array('id'=>$id, 'inst_id'=>$inst_id));
$PAGE->set_url($url);
...
if (!empty($users) && confirm_sesskey()) {
    ...
    foreach ($users as $k => $v) {
        ...
        $note->userid = $v;
        if (note_save($note)) {
            add_to_log($note->courseid, 'notes', 'add', 'index.php?course='.$note->courseid.'&amp;user='.$note->userid . '#note-' . $note->id , 'EWS add note');
        }
    }
