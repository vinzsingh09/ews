...
// requires config file and library file of usp_ews
...
class block_usp_ews extends block_base {
	...
	/**
     * varables used in the block
     *
     * @configureddata has cofiguration setting of the block
     * @monitoreddata has monitored activities data
     * @courseid has current course id
     */
    ...
    public function get_content() {
		...
		/**
		 * Generating the content using standard class object
		 *
		 * @content->text: holds the content of the block
		 */
		...
		$strNote = '<p style="background:#D1EBEF;font-size:8pt; padding:3px; border:1px solid #338c9a;">This is an <strong>experimental</strong> module, developed by FSTE. For any queries or errors regarding this module, please email singh_vn@usp.ac.fj</p>';
		...
		// No modules available, we're done
		if (empty($modules)) {
			...
			if($hasconfigpermission) 
			{
				if(!usp_ews_completion_used($this->courseid)){
					...
					if($USER->editing) // in editing mode
					{
						...
						$urladd = new moodle_url('/blocks/usp_ews/confirm_precondition.php', $params);			
						$label = get_string('config_block', 'block_usp_ews');
						// button to configure selected activities
		...
		if(!usp_ews_completion_used($this->courseid)){
			...
			if($this->configureddata->studentview && $DB->record_exists('role_assignments', array('roleid' =>5, 'userid' => $userid, 'contextid' => $context->id)))  
			{
				/**
